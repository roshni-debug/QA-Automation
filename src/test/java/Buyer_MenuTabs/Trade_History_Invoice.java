/*
    ...
    ==================== TESTCASE STEPS FOR: TRADE HISTORY DOWNLOAD ====================
     ...
     *  7. Click 'Trade History' from sidebar
     *  8. Search 'Buy' and download report
     *  9. Search 'Sell' and download report
...
public class Trade_History_Invoice {
    ...
    @BeforeClass
    public void setup() {
        System.out.println("***************  TestCase Execution for Trade History (Download) Flow  ***************");
        try {
            ...
            prefs.put("download.default_directory", "C:\\Users\\roshn\\Downloads");
            ...
            System.out.println("WebDriver setup successful for TradeHistory test.");
    ...
    public void testTradeHistoryMenu() {
        System.out.println("\n========== Step 8: Click 'Trade History' from sidebar ==========");
        try {
            WebDriverWait sidebarWait = new WebDriverWait(driver, Duration.ofSeconds(10));
            WebElement tradeHistoryMenu = sidebarWait.until(
                ExpectedConditions.elementToBeClickable(By.xpath("//a[normalize-space()='Trade History']")));
            tradeHistoryMenu.click();
        } catch (Exception e) {
            Assert.fail("FAILED [Trade History]: Could not click 'Trade History'. Reason: " + e.getMessage());
        }
    }

    @Test(priority = 9, dependsOnMethods = "testTradeHistoryMenu")
    public void testSearchAndDownloadBuy() throws Exception {
        System.out.println("\n========== Step 9: Search keyword 'Buy' and download report ==========");
        searchAndDownload("Buy");
    }

    @Test(priority = 10, dependsOnMethods = "testSearchAndDownloadBuy")
    public void testSearchAndDownloadSell() throws Exception {
        System.out.println("\n========== Step 10: Search keyword 'Sell' and download report ==========");
        searchAndDownload("Sell");
    ...
    // Utility method for searching a keyword and downloading the trade history report
    private void searchAndDownload(String keyword) throws Exception {
        // Locate search box (dynamic/no id)
        WebElement searchBox = wait.until(ExpectedConditions.elementToBeClickable(
            By.xpath("//input[contains(@placeholder,'Search') or contains(@type,'text')]")));
        searchBox.sendKeys(Keys.CONTROL + "a");
        searchBox.sendKeys(Keys.BACK_SPACE);
        searchBox.sendKeys(keyword);
        System.out.println("Searching: " + keyword);

        // Wait for table or p-table to appear
        wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//table | //p-table")));
        Thread.sleep(1500);

        // Locate and click visible/first download icon (there may be more than one,
        // so be explicit for "Sell")
        List<WebElement> downloadIcons = wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(
            By.xpath("//i[contains(@class,'pi-download') and contains(@class,'text-primary')]")));
        boolean downloaded = false;

        // Try to click the download icon visible after search filtering
        for (WebElement icon : downloadIcons) {
            if (icon.isDisplayed() && icon.isEnabled()) {
                ...
                actions.moveToElement(icon).pause(Duration.ofMillis(200)).click().perform();
                downloaded = true;
                System.out.println("Download clicked for: " + keyword);
                break;
            }
        }
        if (!downloaded) {
            Assert.fail("FAILED [Download]: No download icon found/clicked for: " + keyword);
        }
        Thread.sleep(4000); // Wait for download to complete
    }
    ...
    public static String readTenDigitsFromConsole() {
        ...
        try {
            ...
            // Consume remaining characters of current input line
            int leftover;
