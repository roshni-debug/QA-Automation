...
public class OwnCDs_Cancel {
    ...
    public void testClickCancelButton() {
        ...
        try {
            ...
            WebElement Cancel = button.until(
                ExpectedConditions.elementToBeClickable(By.xpath("(//button[contains(normalize-space(.), 'Cancel')])[2]"))
            );
            Cancel.click();
        } catch (Exception e) {
            Assert.fail("FAILED [Sell Button]: Could not click Cancel button. Reason: " + e.getMessage());
        }
    }
    
    
    @Test(priority = 9, dependsOnMethods = "testClickCancelButton")
    public void testConfirmButton() {
        ...
        try {
            WebDriverWait button = new WebDriverWait(driver, Duration.ofSeconds(10));
            WebElement Confirm = button.until(
                ExpectedConditions.elementToBeClickable(By.xpath("(//button[contains(normalize-space(.), 'Confirm')])[3]"))
            );
            Confirm.click();
        } catch (Exception e) {
            Assert.fail("FAILED [Sell Button]: Could not click on Confirm button. Reason: " + e.getMessage());
        }
    }
    
    @Test(priority = 9, dependsOnMethods = "testConfirmButton")
    public void testContinueButton() {
        ...
        try {
            WebDriverWait button = new WebDriverWait(driver, Duration.ofSeconds(10));
            WebElement Continue = button.until(
                ExpectedConditions.elementToBeClickable(By.xpath("(//button[contains(normalize-space(.), 'Continue')])[3]"))
            );
            Continue.click();
        } catch (Exception e) {
            Assert.fail("FAILED [Sell Button]: Could not click on Continue button. Reason: " + e.getMessage());
        }
    }
