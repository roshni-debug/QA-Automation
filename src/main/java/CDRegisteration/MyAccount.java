...
public class MyAccount {
    ...
    public void testGetMobileNumber() {
        ...
        Assert.assertEquals(mobileNumber.length(), 10, "Mobile number must be 10 digits.");
        System.out.println("\n========== Step 2: Enter Account Number and Retype ==========");
        accountNumber = getAccountNumberFromConsole(scanner);
        Assert.assertTrue(accountNumber.length() >= 10, "Account number looks too short.");
    }

    @Test(priority = 2, dependsOnMethods = "testGetMobileNumber")
    ...
    @Test(priority = 3, dependsOnMethods = "testOpenDigielvAndLogin")
    ...
    @Test(priority = 4, dependsOnMethods = "testFetchAndEnterOtp")
    ...
    @Test(priority = 5, dependsOnMethods = "testHandleKycPopupIfPresent")
    ...
    @Test(priority = 6, dependsOnMethods = "testSelectAccountTypeSavings")
    ...
    @Test(priority = 7, dependsOnMethods = "testEnterAccountNumbers")
    ...
    @Test(priority = 8, dependsOnMethods = "testEnterIfscCode")
    ...
    @Test(priority = 9, dependsOnMethods = "testFileUpload")
    public void testSubmitBankDetails() throws InterruptedException {
        System.out.println("\n========== Step 9: Submit Button Click ==========");
        try {
            WebElement submitButton = new WebDriverWait(driver, Duration.ofSeconds(20)).until(
                ExpectedConditions.elementToBeClickable(
                    By.xpath("//*[@class=\"btn btn-primary ng-star-inserted\"]")
                ));
            ...
            Assert.fail("FAILED [Final Submit]: " + e.getMessage());
        }
    }
