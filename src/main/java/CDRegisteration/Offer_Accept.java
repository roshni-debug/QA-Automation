/*
                ========== TEST CASE STEPS: BID ACCEPTANCE FLOW ==========
     ...
     7. Find all "Accept" buttons and click them (accept all bids).
==================================================================================
*/
...
public class Offer_Accept {
    ...
    public void testClickAllAcceptButtons() {
        System.out.println("\n========== Step 9: Accept all Bids ==========");
        try {
            List<WebElement> acceptButtons = wait.until(
                    ExpectedConditions.visibilityOfAllElementsLocatedBy(
                            By.xpath("//button[contains(@class,'btn-danger') and contains(normalize-space(text()), 'Accept')]")
                    )
            );
            System.out.println("Found " + acceptButtons.size() + " Accept buttons. Clicking...");
            for (WebElement btn : acceptButtons) {
                actions.moveToElement(btn).click().perform();
            }
            System.out.println("All Accept buttons clicked successfully.");
        } catch (Exception e) {
            Assert.fail("FAILED [Accept Buttons]: " + e.getMessage());
        }
    }
